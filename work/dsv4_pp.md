...
`hc_head_fn`、`hc_head_base`、`hc_head_scale` 已挂在 `HcHead` 子模块内部。因此需要一个 DeepSeekV4 专用入口，在 split 前生成正确 stage 列表，保证 `hc_head` 子模块与 `norm/output` 一起保留在 last stage。
...
pipeline_deepseek_v4
  DeepSeekV4 的 PP 总入口，负责生成 stage 列表、split model、调用 parallelize_fn，
  并构建 pipeline schedule。
```
...
2. 非 last stage 不应保留 hc_head 子模块及其内部参数。
...
当前实现已将 `hc_head_fn`、`hc_head_base`、`hc_head_scale` 从
`DeepSeekV4Model` 顶层移动到 `HcHead` 子模块内部：
...
因此 PP split 只要把 `hc_head` 与 `norm/output` 一起放在 last stage，
非 last stage 上的 `hc_head` 子模块会被整体置为 `None`，其内部参数也会随之消失。
不再需要额外实现 `_cleanup_unused_hc_head_parameters()` 来删除顶层
`hc_head_*` 参数。
...
`pp=1` 的正常路径不涉及模型切分，完整模型中 `hc_head` 及其内部参数都会保留。
...
这段断言的作用是尽早暴露切分错误。如果 last stage 没有 `hc_head`，模型仍可能继续跑到 `norm/output`，但语义已经错了；更糟的是错误可能表现为 loss 异常而不是明确 crash。因此 last stage 必须显式检查 `hc_head` module 存在。`hc_head_*` 参数已在 `hc_head` 子模块内部，不需要再检查顶层参数。
